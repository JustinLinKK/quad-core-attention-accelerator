...
# read RTL
analyze -format verilog -lib WORK sfp_optimized.v
...
# Preserve hierarchy: prevent DC from auto-ungrouping sub-designs
...
# Compile - hierarchical (no flatten)
compile_ultra -no_autoungroup -timing_high_effort_script -exact_map

# Write Out Design - Hierarchical netlist (all sub-modules preserved)
current_design $top_module

change_names -rules verilog -hierarchy

write -format verilog -hierarchy -output [format "%s%s" $top_module .out.v]

# Write Reports
redirect [format "%s%s" log/ $top_module _area.rep] { report_area -hierarchy }
...
# Check all sub-designs for unmapped logic (not just top)
foreach_in_collection d [get_designs *] {
    set unmapped_designs [get_designs -filter "is_unmapped == true" [get_attribute $d name]]
