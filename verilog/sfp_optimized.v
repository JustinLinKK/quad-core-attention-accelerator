...
module sfp_optimized (
  ...
  reg [sum_out_bw-1:0] acc_sum_final_q;
  reg [bw_psum-1:0] acc_denom_final_q;
  reg b2_skip_acc_q;
  ...
  assign acc_accept_w = acc && !q_full && !b2_q_full &&
                        !abs_pipe_vld_q && !acc_pair_vld_q && !acc_result_vld_q;
  ...
  always @(posedge clk) begin
    if (reset) begin
      ...
      acc_sum_final_q <= {sum_out_bw{1'b0}};
      acc_denom_final_q <= {bw_psum{1'b0}};
      b2_skip_acc_q <= 1'b0;
      ...
      acc_result_vld_q <= acc_pair_vld_q;
      if (acc_pair_vld_q) begin
        acc_sum_final_q <= acc_sum_final_w;
        acc_denom_final_q <= acc_denom_final_w;
        b2_skip_acc_q <= b2_skip_acc_w;
      end

      if (acc_result_vld_q) begin
        sum_q <= acc_sum_final_q;
        sum_valid_q <= 1'b1;
      end
